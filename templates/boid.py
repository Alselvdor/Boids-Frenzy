...
class Boid(Rules):
    ...
    def draw(self, screen):
        if not self.eaten:
            pg.draw.circle(screen, (80, 150, 100), (int(self.position.x), int(self.position.y)), 20)

    ### Update the position of the boid
    def update(self, boids, hoiks, obstacles=[]):
        if self.eaten:
            return  # Skip the update process for eaten boids
    ...
    #Boghdady's addition
    def delete_boid(self, boids, position_to_delete):
        boids_to_delete = []
        
        for boid in boids:
            if boid.position == position_to_delete:
                boids_to_delete.append(boid)
