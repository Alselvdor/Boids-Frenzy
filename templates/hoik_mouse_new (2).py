...
class NewHoikKeyboard(Rules):
    def __init__(self, screen_width, screen_height, speed_factor):
        ...
        flask_thread = Thread(target=lambda: app.run('0.0.0.0', 9081)) 
        flask_thread.start()

    def draw(self, screen):
        pg.draw.circle(screen, (0, 255, 0), (int(round(self.position.x)), int(round(self.position.y))), int(round(self.size)))
